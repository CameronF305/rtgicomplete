...
{	
	...
	while(ray.length < maxT)
	{
		...
		{
			intersected = saturate(1 - ray.length / maxT); //saturate(1 - lambda);
...
#if MATERIAL_TYPE == 1
		ray.width = min(ray.width, MIP_AMT - saturate(1 - RT_ROUGHNESS * 2.0) * 3); 
#else 
		ray.width = min(ray.width, MIP_AMT); //required for DX9 - mip chains are always generated fully, can't rely on application clamping it
#endif
		//ray.width = 0;
